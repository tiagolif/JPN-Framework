...
for (const relativeDir of assetRoots) {
  ...
  for (const name of entries) {
    ...
    const sourceBuffer = await readFile(path.join(root, relativePath));
...
const digestInput = records
  .map(({ relativePath, bytes, sha256: digest }) => `${relativePath}\t${bytes}\t${digest}`)
  .join('\n');
const sourceStateDigest = sha256(digestInput);
...
const cards = records.map((asset, index) => {
  ...
  return `<article class="card" id="asset-${index + 1}" data-source-sha256="${asset.sha256}">
    ...
    <div class="meta">
      ...
      <span>${asset.bytes} bytes | SHA-256 ${asset.sha256}</span>
...
<p>Estado das fontes: <code>${sourceStateDigest}</code>.</p>
...
console.log(`Visual source state digest: ${sourceStateDigest}`);
