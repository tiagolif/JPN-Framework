...
let blocked=false; try{migrate01to02(unsafe);}catch(error){blocked=/captured_at não pode ser corrigido/.test(error.message);} if(!blocked) throw new Error('Migração insegura deveria falhar explicitamente em vez de inventar timestamp.');
...
console.log('JPN handoff migration OK: baseline 0.1 preservada, conteúdo e imutabilidade protegidos, migração determinística 0.1→0.2 validada e casos inseguros bloqueados.');
