...
const migrate01to02=(source)=>{
  ...
  for(const evidence of source.evidence){
    if(evidence.captured_at!==undefined && evidence.captured_at!==null && !iso.test(evidence.captured_at)) throw new Error(`Migração bloqueada: captured_at não pode ser corrigido sem inventar evidência temporal (${evidence.id})`);
  }
...
if(migrated.contract_version!=='0.2.0-draft'||migrated.evidence[0].claim!==legacy.evidence[0].claim) throw new Error('Migração 0.1→0.2 alterou conteúdo além da versão.');
const unsafe={...legacy,evidence:[{...legacy.evidence[0],captured_at:'20/09/2026 00:00'}]};
if(!validate01(unsafe)) throw new Error('Fixture legada incompatível deveria ser aceita pela baseline 0.1.');
let blocked=false; try{migrate01to02(unsafe);}catch{blocked=true;} if(!blocked) throw new Error('Migração insegura deveria falhar em vez de inventar timestamp.');
console.log('JPN handoff migration OK: baseline 0.1 preservada, migração determinística 0.1→0.2 validada e caso inseguro bloqueado.');
